...
## HSVLR catalog
svs = lapply(c(1:18,'X'), function(chrn){
  message(chrn)
  readSVvcf(paste0('hsvlr-fordedup-', chrn, '.vcf'),keep.ins.seq=T,keep.ref.seq=T,out.fmt=c("vcf") , keep.ids=TRUE)
})
svs = do.call(rbind, svs)

## Remove near-duplicates that were less supported by short-reads
calls.cl = read.table('hsvlr-srdedup18-support.tsv',as.is=TRUE, header=TRUE)
...
write.table(chunks.df, file='graph_srdedup18_augment_chunks.bed', row.names=FALSE, col.names=FALSE, sep='\t', quote=FALSE)
...
writeVcf(svs.for.aug, filename='graph_srdedup18_foraug.vcf')

## Write VCF for construct
info(svs.for.cons)$GT = info(svs.for.cons)$END = info(svs.for.cons)$QUAL = info(svs.for.cons)$SVLEN = NULL
names(geno(svs.for.cons)) = character()
writeVcf(svs.for.cons, filename='graph_srdedup18_forcons.vcf')
