...
Counts = read.table(file_counts,header = TRUE,row.names = 1)
# TPM matrix. Row is gene, column is sample; rowname is gene id, colname is sample id
TPM = read.table(file_tpm,header = TRUE,row.names = 1)

## 1. prepare TMM
samids = colnames(Counts) # sample id
expr_counts <- as.matrix(Counts)  
gene_ids <- rownames(Counts)
expr = DGEList(counts=expr_counts,genes=gene_ids) # counts
...
bed = bed[bed[,2] %in% as.character(1:18),]
bed[,2] = as.numeric(bed[,2])
bed = bed[order(bed[,2],bed[,3]),]
colnames(bed)[2] = "#Chr"
