...
folder_path = '/public/home/yiguoqiang/trans/test'
output_file = 'merged_output.txt'



files = [os.path.join(folder_path, f) for f in os.listdir(folder_path) if f.endswith('.bed')]


with open(os.path.join(folder_path, output_file), 'w') as output:

    output.write('ID\t' + '\t'.join([os.path.basename(file)[:-4] for file in files]) + '\n')
