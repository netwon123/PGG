...
## HSVLR catalog
svs = readSVvcf('graphSV.vcf.gz')
svs_vcf = readSVvcf('graphSV.vcf.gz',out.fmt='vcf',keep.ins.seq=TRUE, keep.ref.seq=TRUE)
names(svs) = paste0('sv', 1:length(svs))
names(svs_vcf) = paste0('sv', 1:length(svs_vcf))
## Make GRanges object
...
write.table(cl.df, file='neardups-clusters_seq.tsv', quote=FALSE, sep='\t', row.names=FALSE)

## Write a VCF with new SV ids for each chromosome
for(chr in unique(as.character(seqnames(svs_vcf)))){
  writeVcf(svs_vcf[which(as.character(seqnames(svs_vcf))==chr)], file=paste0('hsvlr-fordedup-seq', chr, '.vcf'))
}
